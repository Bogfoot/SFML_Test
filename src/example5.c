 ...
 *
 *******************************************************************************/
/* $Id: example5.c,v 1.2 2020/11/25 14:55:45 trurl Exp $ */
...
int run( double threshold )
{
  ...
  collectEvents( 1000, "with MultiStop", &histo1 );
  rc = TDC_preselectSingleStop( 1 );
  checkRc( "TDC_preselectSingleStop", rc );
  collectEvents( 1000, "with SingleStop", &histo2 );
  rc = TDC_preselectSingleStop( 0 );
  checkRc( "TDC_preselectSingleStop", rc );

  printf( "\nStart multistop histogram bins with nonvanishing contents:\n" );
  printf( " bin no   time diff [us]    multistop    singlestop\n" );
  for ( i = 0; i < BINCOUNT; i++ ) {
    if ( histo1 ->values[i] || histo2 ->values[i] ) {
      printf( "%7d %16f %12.1f %12.1f\n", i, i * bin2us, histo1 ->values[i], histo2 ->values[i] );
      filledBins++;
    }
  }
  printf( "\nFilled Bins: %d\n", filledBins );
  ...
  */
  if ( argc <= 1 ) {
    printf( "\nTDC signal conditioning and lifetime feature example.\n\n"
            "Connect a generated signal of ~100kHz to channels start and %d\n"
